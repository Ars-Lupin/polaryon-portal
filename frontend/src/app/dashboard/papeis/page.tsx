...
export default function PapeisPage() {
  ...
  return (
    <ProtectedRoute requiredPermission="papeis.ler">
      <div className="mx-auto flex max-w-7xl flex-col gap-6">
        <div>
          ...
          <p className="mt-2 text-sm text-slate-500 dark:text-slate-300">
            Papéis determinam quais permissões cada usuário possui.
          </p>
        </div>
        ...
        {loading ? (
          ...
          <div className="grid animate-fadeIn gap-4 md:grid-cols-2 xl:grid-cols-3">
            {papeis.map((papel) => (
              <article key={papel.id} className="polaryon-card rounded-3xl p-6">
